...
def _validate_once(
    ...
    baseline_region_ari_mean = region_scores.drop(index=hgt_region_col).mean()
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"Method B (per-region): {'PASS' if method_b_ok else 'FAIL'}")
    return 0 if (method_a_ok or method_b_ok) else 1
